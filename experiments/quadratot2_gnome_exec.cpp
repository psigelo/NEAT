...
double experiment( Genetic_Encoding organism ,int handle_joints[N_LEGS*GRA_LIB + GRA_LIB_EXT], int handle_tips[N_LEGS], int * handle_dummy, int * clientID){
...
int main(int argc, char** argv){
	...
	if(argc < 2){
		cerr << "Arguments missing, The relative path to the genetic encoding files must be defined!. \n";
		exit(1);
	}
	...
	cerr << organism << endl;
	experiment(organism, handle_joints, handle_tips, &handle_dummy, &clientID);
